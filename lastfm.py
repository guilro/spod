...
lastfm_arg_parser.add_argument(
    "--lastfm_username", help="your Last.fm username", default=None
)
lastfm_arg_parser.add_argument(
    "--lastfm_password", help="your Last.fm password", default=None
)
lastfm_arg_parser.add_argument(
    "--lastfm_api_key", help="your Last.fm API key", default=None
)
lastfm_arg_parser.add_argument(
    "--lastfm_api_secret", help="your Last.fm API secret", default=None
)
lastfm_arg_parser.add_argument(
    "--lastfm_credentials",
    help="file to load Last.fm credentials from",
    default="lastfm_credentials.json",
)
...
class LastFM:
    def __init__(self):
        self.credentials = dict(
            {"username": None, "password": None, "api_key": None, "api_secret": None}
        )
    ...
    def pause(self):
        ...
        if self.playing:
            print(
                "LastFM: add "
                + str(time.time() - self.play_beg)
                + " to total played time"
            )
    ...
    def track_changed(self):
        ...
        if self.metadata and self.play_cumul > min(
            ...
            print(
                "LastFM: scrobbled track "
                + self.metadata["track_name"].decode()
                + " - "
                + self.metadata["artist_name"].decode()
            )
        ...
        self.lastfm_network.update_now_playing(
            artist=self.metadata["artist_name"].decode(),
            title=self.metadata["track_name"].decode(),
            album=self.metadata["album_name"].decode(),
