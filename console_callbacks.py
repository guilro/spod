...
playback_led = gpiozero.LED(17)
play_pause_button = gpiozero.Button(2)
...
playback_device_group.add_argument(
    "--device",
    "-D",
    help="alsa output device (deprecated, use --playback_device)",
    default="default",
)
playback_device_group.add_argument(
    "--playback_device",
    "-o",
    help="alsa output device (get name from aplay -L)",
    default="hw",
)

audio_arg_parser.add_argument(
    "--mixer_device_index", help="alsa card index of the mixer device", type=int
)
audio_arg_parser.add_argument(
    "--mixer", "-m", help="alsa mixer name for volume control", default=alsa.mixers()[0]
)
audio_arg_parser.add_argument(
    "--dbrange", "-r", help="alsa mixer volume range in Db", default=0
)
...
class AlsaSink:
    ...
    def acquire(self):
        if self._session.is_active():
            try:
                pcm_args = {"type": alsa.PCM_PLAYBACK, "mode": alsa.PCM_NORMAL}
...
def connection_new_credentials(self, blob):
    ...
    self.credentials["blob"] = str(ffi.string(blob))
...
@ffi.callback(
    "uint32_t(const void *data, uint32_t num_samples, SpSampleFormat *format, uint32_t *pending, void *userdata)"
)
@userdata_wrapper
...
def playback_volume(self, volume):
    ...
    if volume == 0:
        ...
        corected_playback_volume = int(
            min_volume_range + ((volume / 655.35) * (100 - min_volume_range) / 100)
        )
        print("corected_playback_volume: {}".format(corected_playback_volume))
        mixer.setvolume(corected_playback_volume)


connection_callbacks = ffi.new(
    "SpConnectionCallbacks *", [connection_notify, connection_new_credentials]
)

debug_callbacks = ffi.new("SpDebugCallbacks *", [debug_message])

playback_callbacks = ffi.new(
    "SpPlaybackCallbacks *",
